# 쿠팡친구 배송 장부 (Coupang Delivery Bookkeeping)

쿠팡친구 화물 개인사업자를 위한 일일 장부 관리 도구.
AI 에이전트(Hermes Agent)와 연동하여 자연어로 매출/경비/유류비를 기록하고, 홈택스 셀프 신고용 데이터를 생성합니다.

## 특징

- **자연어 입력** — "804C 150개, LPG 1100원 35리터" → 자동 파싱 및 기록
- **구역별 단가 자동 적용** — 804C(1,050원), 804D(850원), 901CD(1,000원)
...
구역이 추가되면 `bookkeeper.py`의 `ZONE_PRICES` 딕셔너리에 추가하세요.
