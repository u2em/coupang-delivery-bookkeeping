...
- **자연어 입력** — "151C 150개, LPG 1100원 35리터" → 자동 파싱 및 기록
- **구역별 단가 자동 적용** — 151C(1,100원), 151D(850원), 152C/152D(1,100원); 이전 804/901 코드 보존
- **LPG 유가보조금 자동 차감** — 115.14원/L (2026-08-03 충전분 및 유가보조금 앱·공식 사이트 확인)
...
```bash
# LPG 주유 — 유가보조금(115.14원/L) 자동 차감
# 2026-08-03 충전분과 유가보조금 앱·공식 사이트에서 확인한 기본값
python3 bookkeeper.py add-fuel --price-per-liter 1100 --liters 35
# → 총 38,500원 - 보조금 4,030원 = 실 경비 34,470원
```
...
# 구역 단가 수정
python3 bookkeeper.py update-zone --code 151C --unit-price 1100
...
```
사용자: "오늘 151C 150개 151D 100개 했고 분실 하나 만오천원, 자동차검사 5만원, LPG 천백원에 35리터"

에이전트:
  📋 2026-04-14 장부 기록 완료

  매출:
    151C  150건 × 1,100원 = 165,000원
    151D  100건 ×   850원 =  85,000원
...
| 구역코드 | 단가 | 캠프·적용 |
|---------|------|-----------|
| 151C | 1,100원 | F_삼선1, 2026-07-29부터 (이전 804C) |
| 151D | 850원 | F_삼선1, 2026-07-29부터 (이전 804D) |
| 152C | 1,100원 | F_삼선1, 2026-07-29부터 (이전 901C) |
| 152D | 1,100원 | F_삼선1, 2026-07-29부터 (이전 901D) |

과거 장부 조회와 소급 입력을 위해 중구1의 804C/804D/901C/901D 코드도 보존합니다.
구역은 DB의 `zone` 테이블에서 관리됩니다. 아래 구역 관리 명령을 사용하세요.

## 세무 참고

- 일반과세자 기준
- LPG 유가보조금(115.14원/L, 2026-08-03 충전분 및 유가보조금 앱·공식 사이트 확인)은 환급 수입 → 경비에서 차감
